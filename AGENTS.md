...
- Prefer a recognizable tool-generated default over a locally "simplified" rewrite. Standard code has near-zero cognitive load.
...
## Defaults And Generated Code

- Preserve standard files generated by Bun, shadcn, and other chosen tools unless a product requirement forces a change.
- Do not rewrite generated code merely to satisfy local style, line-count, or abstraction preferences.
- Treat familiar upstream structure as simpler than bespoke code, even when the upstream file is longer.
- Keep necessary changes to generated files narrow so future readers can still recognize the original.
- Configure lint around intentional upstream patterns instead of churning generated source.
- Remove generated code only when the generating tool expects consumers to curate it and the code has no current use.
